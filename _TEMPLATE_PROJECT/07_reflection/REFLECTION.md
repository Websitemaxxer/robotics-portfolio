# Reflection — [PROJECT NAME]

Write this at the end, while it's fresh. Honest reflection is one of the strongest
signals in a portfolio — it shows you can evaluate your own work.

- **What was original?** What did I design, write, or figure out myself?
  > <fill in>

- **What was adapted, and from where?** Kits, tutorials, open-source designs, code
  snippets — name them and link them.
  > <fill in>

- **Hardest problem + how I solved it.** The one that nearly stopped me.
  > <fill in>

- **What I'd do differently next time.**
  > <fill in>

- **What transfers to the next project?** Skills, tools, or habits I'll reuse.
  > <fill in>

- **What skills does this project prove?** (Tick the matching boxes in the top-level
  `README.md` Skills checklist too.)
  > <fill in>
